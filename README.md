...
## パッケージ説明
*   **`game_server/`**: ゲームのコア機能実現______游戏核心功能
*   **`gateway/`**: ログイン校验とクライアントメッセージの検証・転送______账号校验和验证转发客户端的消息
*   **`protos/`**: game_serverとgateway公用のプロトコル＆生成スクリプト______game_server和gateway公用的协议和生成脚本
